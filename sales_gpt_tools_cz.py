...
from sales_tools import CustomPromptTemplate, CustomToolsChain, CustomOutputParser
...
class SalesGPT(Chain, BaseModel):
    ...
    tools_chain: CustomToolsChain = Field(...) # TOOLS
    #execution_chain: AgentExecutor = Field(...)
    #tools_action_chain : ToolsActionChai = Field(...)
    ...
    def check_tools(self):
      func_id = self.tools_chain.run(
          ...
          objective = "Pokud zjistíś jméno zákazníka, najdi informace o něm" ,
          input = "Zjisti věk zákazníka a vynásob ho deseti"  ,
          intermediate_steps = ''
          )
      return f"FUNC: {func_id}"
    ...
    def from_llm(
        ...
        tools_template = deepcopy(kwargs['custom_tools_prompt'])
        tools_chain = CustomToolsChain.from_llm(llm, verbose=verbose, template = tools_template ) # TOOLS

        return cls(
            stage_analyzer_chain=stage_analyzer_chain,
            sales_conversation_utterance_chain=sales_conversation_utterance_chain,
            #execution_chain=agent_executor,
            tools_chain = tools_chain, # TOOLS
            verbose=verbose,
            **kwargs,
        )
