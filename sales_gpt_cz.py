...
class StageAnalyzerChain(LLMChain):
    ...
    @classmethod
    @time_logger
    def from_llm(cls, llm: BaseLLM, verbose: bool = True, custom_stage_prompt : str = "") -> LLMChain:
        """Get the response parser."""
        stage_analyzer_inception_prompt_template = custom_stage_prompt
        """(
...
class SalesConversationChain(LLMChain):
    ...
    def from_llm(cls, llm: BaseLLM, 
        ...
        if use_custom_prompt:
           ...
           logger.error("missing custom_prompt")
...
class SalesGPT(Chain, BaseModel):
    ...
    def from_llm(
        ...
        #global CONVERSATION_STAGES
        if 'custom_stage_prompt' in kwargs.keys(): 
            custom_stage_prompt = deepcopy(kwargs['custom_stage_prompt'])
            logger.info(f"USING custom stage prompt   {custom_stage_prompt}")
        else: 
            logger.error(f"ERROR custom_stage_prompt missing ${kwargs}")
 
        stage_analyzer_chain = StageAnalyzerChain.from_llm(llm, verbose=verbose, custom_stage_prompt = custom_stage_prompt)
        ...
        if 'conversation_stages' in kwargs.keys():  # test existence v json config - jinak vezmu dict z konstanty
            ...
            logger.info(f"USING custom conv stages from json {conversation_stages}")
        else: 
            logger.error(f"ERROR conversation_stages missing ${kwargs}")
